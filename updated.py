...
# Optional Supabase — graceful fallback to local file if not configured
try:
...
def load_users() -> pd.DataFrame:
    ...
    if client:
        try:
            resp = client.table("users").select(",".join(_USER_COLS)).execute()
            if resp.data:
                return pd.DataFrame(resp.data)[_USER_COLS]
            # Table is empty on first deploy — seed default accounts
            defaults = _default_users()
            client.table("users").insert(defaults).execute()
            return pd.DataFrame(defaults)
        except Exception as e:
            st.warning(f"Database read error: {e}. Falling back to local file.")
...
def save_users(df: pd.DataFrame) -> None:
    ...
    if client:
        try:
            ...
            client.table("users").upsert(records, on_conflict="username").execute()
            return
        except Exception as e:
            st.warning(f"Database write error: {e}. Saving to local file instead.")
